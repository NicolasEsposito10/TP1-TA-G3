...
void Device::controlVentilacion() {
    if (temp > refTemp) {
        if (!ventiladorActivo) Serial.println("Ventilacion ACTIVADA");
        ventiladorActivo = true;
        digitalWrite(ledVent, HIGH);
    } else {
        if (ventiladorActivo) Serial.println("Ventilacion DESACTIVADA");
        ventiladorActivo = false;
...
void Device::controlRiego() {
    if (hum < humedadUmbral) {
        if (!riegoActivo) Serial.println("Riego ACTIVADO");
        riegoActivo = true;
        ...
        if (riegoActivo) Serial.println("Riego DESACTIVADO");
        riegoActivo = false;
